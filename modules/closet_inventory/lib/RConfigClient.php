 ...
 */
/**
 * Marker exception class for callers that want to special-case ambiguous
 * device-resolution failures. The vendored resolveDeviceId() still throws
 * \RuntimeException (logic preserved verbatim); callers may match on the
 * "multiple rConfig devices match" substring in $msg, or wrap their own
 * lookup that re-throws as this class.
 */
class RConfigAmbiguousMatchException extends \RuntimeException {}
...
class RConfigClient {
    ...
    /**
     * Return the latest config-backup metadata for a device, as
     * { id, name, lastBackupAt (ISO-8601|null), lastBackupAgeDays (int|null) }.
     *
     * Best-effort: queries GET /api/v2/devices/{id}/configs (newest first) and
     * falls back to GET /api/v2/devices/{id} for the device name. Any field
     * we cannot parse stays null so the caller (EnrichmentService) can render
     * a "no backup info" chip rather than crashing.
     *
     * @return array{id:int,name:string,lastBackupAt:?string,lastBackupAgeDays:?int}
     */
    public function getDeviceConfigBackupInfo(int $deviceId): array {
        if ($deviceId <= 0) {
            return ['id' => $deviceId, 'name' => '', 'lastBackupAt' => null, 'lastBackupAgeDays' => null];
        }

        $name = '';
        try {
            [$st, $dev] = $this->raw('GET', '/api/v2/devices/'.$deviceId, [], null);
            if ($st >= 200 && $st < 300) {
                $row = $dev['data'] ?? $dev;
                if (is_array($row)) {
                    $name = (string) ($row['device_name'] ?? $row['name'] ?? '');
                }
            }
        }
        catch (\Throwable $e) {
        ...
        $ageDays  = null;
        try {
            [$st, $payload] = $this->raw('GET', '/api/v2/devices/'.$deviceId.'/configs', [
                'per_page' => 1,
                'page'     => 1,
                'sort'     => '-created_at'
            ], null);
            ...
            if ($st >= 200 && $st < 300) {
                ...
                if (is_array($rows) && isset($rows[0]) && is_array($rows[0])) {
                    $r  = $rows[0];
                    $ts = $r['created_at'] ?? $r['updated_at'] ?? $r['backup_at'] ?? null;
                    if (is_string($ts) && $ts !== '') {
                        $t = strtotime($ts);
                        if ($t !== false && $t > 0) {
                            $lastIso = gmdate('c', $t);
                            $ageDays = (int) floor((time() - $t) / 86400);
            ...
            // Caller surfaces the source as "down".
            throw $e;
        }
